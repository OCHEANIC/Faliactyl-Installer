...
file_install() {
    ...
    echo "======================================================"
    cd /var/www/
    sudo git clone https://github.com/Evolution-Development/Faliactyl
    cd Faliactyl
    sudo npm install
    sudo npm install forever -g
    echo "======================================================"
    echo "File Installaiton Complete"
    echo "======================================================"
}
settings_configuration() {
    echo "======================================================"
    ...
    echo "https://faliactyl.prajwaldev.xyz/"
    echo "======================================================"
    cd /var/www/Faliactyl/
    file=settings.json

    echo "What is the web port? [8080] (Port on which Faliactyl Works)"
    read WEBPORT
    echo "What is the web secret? (This will be used for logins)"
    read WEB_SECRET
    echo "What is the pterodactyl domain? [EG - https://panel.yourdomain.com]"
    read PTERODACTYL_DOMAIN
    echo "What is the pterodactyl key? [ With All Perms]"
...
reverseproxy_configuration() {
   ...
   read DOMAIN
   apt install nginx
   sudo wget -O /etc/nginx/conf.d/faliactyl.conf https://raw.githubusercontent.com/OCHEANIC/Faliactyl-Installer/main/NginxHTTPReverseProxy.conf
   ...
   sudo nginx -t
   echo "-------------------------------------------------------"
   echo "Note: if it does not say OK in the line, there's a error."
   echo "-------------------------------------------------------"
...
update_check() {
    latest=$(wget https://raw.githubusercontent.com/OCHEANIC/Faliactyl-Installer/main/version.json -q -O -)
